...
function findPinkElephantsByTimestreet(songs) {
  return songs.find((elements) =>  elements.title === 'Pink Elephants')
}
...
function findFirstSongUnderThreeMinutes(songs) {
  return songs.find((songUnderThreeMin) =>  songUnderThreeMin.runtimeInSeconds < 180);
}
...
function findFirstTitleTrack(songs) {
  return songs.find((firstTrack) =>  firstTrack.title === firstTrack.album);
}
